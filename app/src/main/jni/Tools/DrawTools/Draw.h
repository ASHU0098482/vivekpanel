...
public:
    ...
    void DrawSignatureText(Color color, const char *str, Vector2 pos, float size) {
        if (isValid() && str != nullptr) {
            _env->CallVoidMethod(_cvsView, _env->GetMethodID(_env->GetObjectClass(_cvsView), "DrawSignatureText", "(Landroid/graphics/Canvas;IIIILjava/lang/String;FFF)V"), _cvs, (int) color.a, (int) color.r, (int) color.g, (int) color.b, _env->NewStringUTF(str), pos.X, pos.Y, size);
        }
        return;
    }
    ...
    void DrawLogo(float posX, float posY, float width, float height, float alpha) {
        if (isValid()) {
            _env->CallVoidMethod(_cvsView,
                                 _env->GetMethodID(_env->GetObjectClass(_cvsView), "DrawLogo",
                                                   "(Landroid/graphics/Canvas;FFFFF)V"),
                                 _cvs, posX, posY, width, height, alpha);
        }
        return;
    }
    ...
    void DrawBlackScreen(int alpha) {
        if (isValid()) {
            _env->CallVoidMethod(_cvsView,
                                 _env->GetMethodID(_env->GetObjectClass(_cvsView), "DrawFilledRectInfo",
                                                   "(Landroid/graphics/Canvas;IIIIFFFF)V"),
                                 _cvs, alpha, 0, 0, 0, 0.0f, 0.0f, (float)getWidth(), (float)getHeight());
        }
        return;
    }
